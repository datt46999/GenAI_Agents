"""
This script demonstrate how to create simple MCP server that feshes 
that current price of cryptocurrent using CoinGecko API.
It uses FactMCP libarary to create server and handle requests
"""
import httpx
...
async def get_crypto_market_info(crypto_ids: str, currency: str = "usd")->str:
    """
    Get market information  for one or more cryptocurrency 
    Parameters:
    crypto_id: Comma-separate list of cryptocurrency IDs (e.g.,'bitcoin, enthereum')
    currency: the currency to deplay values in (default:'usd')
    return : 
    Market information includiing price, market cap, volume, and price changes
    """
    # Construct the API URL
